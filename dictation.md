...
- ONEPIECE Framework has units for database, form, ORM, SQL, validation, and related model-style work. Keep `AGENTS.md` lightweight by linking to the compact unit list in `asset/docs/unit/available-units.md`; agents should use the responsible unit and its public API or mapping instead of hand-rolling a parallel application layer.
- The Form unit is called as `OP()->Unit()->Form()` and its source is `asset/unit/form`; validation belongs to the Validate unit, not the Form unit.
- When creating agent-facing feature docs, separate ordinary usage from source-derived current specs to save context. Agents using CORE features or units should read `usage.md`; agents debugging source-level problems or refactoring should read `current-spec.md`. Add focused `troubleshooting.md` documents when common failures need a smaller diagnostic path.
